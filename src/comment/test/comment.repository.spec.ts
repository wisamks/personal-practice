...
describe('CommentRepository', () => {
  ...
  describe('findCommentsByPostId, 입력: 댓글 쿼리, 동작: 댓글 페이지네이션 및 반환', () => {
    it('반환: 여러 댓글 객체 배열, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.findCommentsByPostId({ postId, take, cursor })).resolves.toEqual<Comment[]>(
        foundComments,
      );
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.findCommentsByPostId({ postId, take, cursor })).rejects.toThrow(
  ...
  describe('findComment, 입력: 댓글 아이디, 동작: 단일 댓글 반환', () => {
    it('반환: 단일 댓글 객체, 조건: db 통신 성공', async () => {
      ...
      expect(mockPrismaService.comment.findUnique).toHaveBeenCalledWith<[Prisma.CommentFindUniqueArgs]>({
        where,
      });
    });
  ...
  describe('updateComment, 입력: 댓글 수정 데이터 & 댓글 아이디, 동작: 단일 댓글 수정 후 결과 반환', () => {
    it('반환: 수정 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.updateComment({ data, commentId, userId })).resolves.toEqual<Prisma.BatchPayload>(
        updatedResult,
      );
      expect(mockPrismaService.comment.updateMany).toHaveBeenCalledWith<[Prisma.CommentUpdateManyArgs]>({
        data,
        where,
      });
    });
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.updateComment({ data, commentId, userId })).rejects.toThrow(
        RepositoryBadGatewayException,
      );
      expect(mockPrismaService.comment.updateMany).toHaveBeenCalledWith<[Prisma.CommentUpdateManyArgs]>({
        data,
  ...
  describe('deleteComment, 입력: 댓글 아이디, 동작: 단일 댓글 삭제 후 결과 반환', () => {
    it('반환: 삭제 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.deleteComment({ commentId, userId })).resolves.toEqual<Prisma.BatchPayload>(
        deletedResult,
      );
      expect(mockPrismaService.comment.updateMany).toHaveBeenCalledWith<[Prisma.CommentUpdateManyArgs]>({
        where,
