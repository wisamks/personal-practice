...
describe('CommentController', () => {
  ...
  describe('getCommentsByCursor, 입력: 게시글 아이디 & 댓글 쿼리, 동작: 전달 후 반환', () => {
    it('반환: 여러 댓글 객체 배열, 조건: 항상', async () => {
      ...
      await expect(controller.getCommentsByCursor(getCommentsReqDto, postId)).resolves.toEqual<GetCommentResDto[]>(
        foundComments,
      );
      expect(service.getCommentsByPostId).toHaveBeenCalledWith<
        [{ getCommentsReqDto: GetCommentsReqDto; postId: number }]
      >({ getCommentsReqDto, postId });
    });
  });

  describe('createComment, 입력: 댓글 생성 데이터 & 유저 아이디 & 게시글 아이디, 동작: 전달 후 반환', () => {
    it('반환: 댓글 아이디 객체, 조건: 항상', async () => {
      const createdComment: CreateCommentResDto = plainToInstance(CreateCommentResDto, mockComment1);
      ...
      await expect(controller.createComment(createCommentReqDto, postId, userId)).resolves.toEqual<CreateCommentResDto>(
        createdComment,
      );
      ...
      >({ createCommentReqDto, postId, userId });
    });
  });
  ...
  describe('updateComment, 입력: 댓글 수정 데이터 & 유저 아이디 & 게시글 아이디 & 댓글 아이디, 동작: 전달', () => {
    it('반환: undefined, 조건: 항상', async () => {
      ...
      await expect(controller.updateComment(updateCommentReqDto, postId, commentId, userId)).resolves.toEqual<void>(
        undefined,
      );
  ...
  describe('deleteComment, 입력: 댓글 아이디 & 게시글 아이디 & 유저 아이디, 동작: 전달 후 반환', () => {
    it('반환: 다음 댓글 객체, 조건: 항상', async () => {
      ...
      expect(service.deleteComment).toHaveBeenCalledWith<[{ commentId: number; postId: number; userId: number }]>({
        commentId,
        postId,
        userId,
      });
