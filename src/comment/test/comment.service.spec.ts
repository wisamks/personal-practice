...
describe('CommentService', () => {
  ...
  describe('getCommentsFirstPage, 입력: 게시글 아이디 & 댓글 쿼리, 동작: 첫 페이지 댓글 반환', () => {
    it('반환: 여러 댓글 배열, 조건: 레디스에 없을 때, 동작: 레디스에 저장 후 반환', async () => {
      ...
      await expect(service.getCommentsFirstPage({ getCommentsReqDto, postId })).resolves.toEqual<Comment[]>(
        foundComments,
      );
      expect(redisClient.get).toHaveBeenCalledWith<[string]>(commentsKey);
      expect(repository.findCommentsByPostId).toHaveBeenCalledWith<[IGetCommentsInput]>({
        ...getCommentsReqDto,
        postId,
      });
      expect(redisClient.set).toHaveBeenCalledWith<[string, string, string, number]>(
  ...
  describe('getCommentsByPostId, 입력: 게시글 아이디 & 댓글 쿼리, 동작: 여러 댓글 페이지별 반환', () => {
    it('반환: 여러 댓글 객체 배열, 조건: 항상', async () => {
      ...
      await expect(service.getCommentsByPostId({ getCommentsReqDto, postId })).resolves.toEqual<GetCommentResDto[]>(
        plainToInstance(GetCommentResDto, foundComments),
      );
      expect(repository.findCommentsByPostId).toHaveBeenCalledWith<[IGetCommentsInput]>({
        ...getCommentsReqDto,
        postId,
  ...
  describe('createComment, 입력: 댓글 생성 데이터 & 게시글 아이디 & 유저 아이디', () => {
    it('반환: 생성 댓글 아이디 객체, 조건: 레디스에 댓글 수가 있을 때, 동작: 댓글 생성 및 댓글 수 증가 후 반환', async () => {
      ...
      await expect(
        service.createComment({ createCommentReqDto, postId, userId }),
      ).resolves.toEqual<CreateCommentResDto>(plainToInstance(CreateCommentResDto, createdComment));
      expect(repository.createComment).toHaveBeenCalledWith<[ICreateCommentInput]>({
        ...createCommentReqDto,
        postId,
        authorId: userId,
      });
  ...
  describe('updateComment, 입력: 댓글 수정 데이터 & 유저 아이디 & 게시글 아이디 & 댓글 아이디, 동작: 댓글 수정', () => {
    it('반환: undefined, 조건: 수정 성공', async () => {
      ...
      ).resolves.toEqual<void>(undefined);
      expect(redisClient.del).toHaveBeenCalledWith<[string]>(commentsKey);
      ...
      >({ data: updateCommentReqDto, commentId, userId });
    ...
    it('반환: ForbiddenError, 조건: 권한 없음', async () => {
      ...
      >({ data: updateCommentReqDto, commentId, userId: 2 });
      expect(repository.findComment).toHaveBeenCalledWith<[number]>(commentId);
    });
  });
  ...
  describe('deleteComment, 입력: 댓글 아이디 & 유저 아이디 & 게시글 아이디, 동작: 댓글 삭제 후 다음 댓글 반환', () => {
    it('반환: 다음 댓글 객체, 조건: 삭제 성공', async () => {
      ...
      await expect(service.deleteComment({ commentId, userId, postId })).resolves.toEqual<GetCommentResDto>(
        plainToInstance(GetCommentResDto, nextComment),
      );
      expect(repository.findCommentsByPostId).toHaveBeenCalledWith<[IGetCommentsInput]>({
        postId,
        cursor: commentId,
