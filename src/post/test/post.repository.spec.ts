...
describe('PostRepository', () => {
    ...
    describe('findPosts, 입력: 페이지네이션 쿼리, 동작: 여러 게시글 찾아서 반환', () => {
        it('반환: 여러 게시글 객체 배열, 조건: db 통신 성공', async () => {
            ...
            expect(mockPrismaService.post.findMany).toHaveBeenCalledWith<[Prisma.PostFindManyArgs]>({ where, orderBy, skip: 0, take, include: postsInclude });
        });
    ...
    describe('findPost, 입력: 게시글 아이디, 동작: 단일 게시글 찾아서 반환', () => {
        it('반환: 단일 게시글 객체, 조건: db 통신 성공', async () => {
            ...
            expect(mockPrismaService.post.findUnique).toHaveBeenCalledWith<[Prisma.PostFindUniqueArgs]>({ where, include: postInclude });
        });
    ...
    describe('createPost, 입력: 트랜잭션 객체 & 생성 데이터, 동작: 단일 게시글 생성 후 반환', () => {
        it('반환: 단일 게시글 객체, 조건: db 통신 성공', async () => {
            ...
            expect(mockTx.post.create).toHaveBeenCalledWith<[Prisma.PostCreateArgs]>({ data });
        });
    ...
    describe('updatePost, 입력: 수정 데이터 & 게시글 아이디 & 유저 아이디, 동작: 게시글 수정 후 결과 반환', () => {
        it('반환: 수정 결과 개수 객체, 조건: db 통신 성공', async () => {
            ...
            await expect(repository.updatePost(mockTx as unknown as Prisma.TransactionClient, { data, postId, userId }))
    ...
    describe('deletePost, 입력: 트랜잭션 객체 & 유저 아이디 & 게시글 아이디, 동작: 삭제 후 결과 반환', () => {
        it('반환: 삭제 결과 개수 객체, 조건: db 통신 성공', async () => {
            ...
            await expect(repository.deletePost(mockTx as unknown as Prisma.TransactionClient, { userId, postId }))
