...
import {
  ...
  PostInternalServerErrorException,
...
export class PostService {
  ...
  async getPostsByCursor(getCursorReqDto: GetCursorReqDto): Promise<GetPostResDto[]> {
    ...
    for (const foundPost of foundPosts) {
      const [foundTags, commentsCount, likesCount, viewsCount] = await Promise.all([
  ...
  async getPost({ postId, userId }: { postId: number; userId: number }): Promise<GetPostResDto> {
    ...
    const [foundTags, foundComments, commentsCount, likesCount, viewsCount] = await Promise.all([
  ...
  async createPost({
    ...
    return await this.prismaService.$transaction(async (tx) => {
      const createdPost = await this.postRepository.createPost(tx, {
        title,
        content,
        authorId: userId,
      });
      if (tags && createdPost) {
        await this.tagService.createTags(tx, {
          tags,
  ...
  async updatePost({
    ...
    await this.prismaService.$transaction(async (tx) => {
      const updatedResult = await this.postRepository.updatePost(tx, {
        data: { title, content },
        postId,
        userId,
      });
  ...
  async deletePost({ postId, userId }: IDeletePostQuery): Promise<void> {
    ...
    return await this.prismaService.$transaction(async (tx) => {
      const deletedResult = await this.postRepository.deletePost(tx, {
        postId,
        userId,
      });
