...
describe('PostLikeRepository', () => {
  ...
  describe('findPostLikeByPostAndUser, 입력: 게시글 아이디 & 유저 아이디, 동작: 단일 좋아요 반환', () => {
    it('반환: 단일 좋아요 객체, 조건: db 통신 성공', async () => {
      ...
      expect(mockPrismaService.postLike.findFirst).toHaveBeenCalledWith<[Prisma.PostLikeFindFirstArgs]>({
        where,
      });
    });
  ...
  describe('createPostLikes, 입력: 게시글 아이디 & 유저 아이디 배열, 동작: 여러 좋아요 생성 후 결과 반환', () => {
    it('반환: 생성 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      expect(mockPrismaService.postLike.createMany).toHaveBeenCalledWith<[Prisma.PostLikeCreateManyArgs]>({
        data,
      });
    });
  ...
  describe('deletePostLike, 입력: 좋아요 아이디, 동작: 단일 좋아요 삭제', () => {
    it('반환: undefined, 조건: db 통신 성공', async () => {
      ...
      expect(mockPrismaService.postLike.update).toHaveBeenCalledWith<[Prisma.PostLikeUpdateArgs]>({
        where,
        data,
      });
    });
  ...
  describe('deletePostLikes, 입력: 게시글 아이디 & 유저 아이디 배열, 동작: 여러 좋아요 삭제 후 결과 반환', () => {
    it('반환: 삭제 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.deletePostLikes({ postId, userIds })).resolves.toEqual<Prisma.BatchPayload>(
        deletedResult,
      );
      expect(mockPrismaService.postLike.updateMany).toHaveBeenCalledWith<[Prisma.PostLikeUpdateManyArgs]>({
        where,
        data,
      });
    });
