...
describe('UserRepository', () => {
  ...
  describe('findUserByProviderOptions, 입력: provider options 객체, 동작: 단일 유저를 찾아서 반환', () => {
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.findUserByProviderOptions(providerOptions)).rejects.toThrow(
  ...
  describe('updateUser, 입력: 유저 수정 data & 유저 아이디, 동작: 유저 수정 후 결과 반환', () => {
    it('반환: 수정 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.updateUser({ updateUserReqDto: data, userId })).resolves.toEqual<Prisma.BatchPayload>(
        updatedResult,
      );
      expect(mockPrismaService.user.updateMany).toHaveBeenCalledWith<[Prisma.UserUpdateManyArgs]>({
        data,
        where,
      });
    });
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.updateUser({ updateUserReqDto: data, userId })).rejects.toThrow(
        RepositoryBadGatewayException,
      );
      expect(mockPrismaService.user.updateMany).toHaveBeenCalledWith<[Prisma.UserUpdateManyArgs]>({
        data,
  ...
  describe('updateUserCreateRefresh, 입력: 리프레시 토큰 & 유저 아이디, 동작: 유저 테이블에 리프레시 토큰 저장 후 결과 반환', () => {
    it('반환: 수정 결과 개수 반환, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.updateUserCreateRefresh({ refreshToken, userId })).resolves.toEqual<Prisma.BatchPayload>(
        updatedResult,
      );
      expect(mockPrismaService.user.updateMany).toHaveBeenCalledWith<[Prisma.UserUpdateManyArgs]>({
        where,
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.updateUserCreateRefresh({ refreshToken, userId })).rejects.toThrow(
        RepositoryBadGatewayException,
      );
      expect(mockPrismaService.user.updateMany).toHaveBeenCalledWith<[Prisma.UserUpdateManyArgs]>({
        where,
