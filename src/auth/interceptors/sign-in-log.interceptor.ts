import { CallHandler, ExecutionContext, HttpStatus, Injectable, InternalServerErrorException, Logger, NestInterceptor } from "@nestjs/common";
import { catchError, Observable, tap } from "rxjs";

@Injectable()
export class SignInLogInterceptor implements NestInterceptor {
    private readonly logger = new Logger(SignInLogInterceptor.name);
    ...
    intercept(context: ExecutionContext, next: CallHandler<any>): Observable<any> | Promise<Observable<any>> {
        ...
        return next.handle().pipe(
            tap(() => {                
                ...
                this.logger.verbose(`${method} ${statusCode} ${url} ${delay}ms`);
            }), 
            catchError((err) => {
                const delay = Date.now() - now;
