...
describe('AuthController', () => {
    ...
    describe('kakaoCallback, 입력: oauth 유저 데이터 & 응답 객체, 동작: 토큰을 받아 쿠키에 저장', () => {
        it('반환: undefined, 조건: 항상', async () => {
            ...
            expect(mockRes.cookie).toHaveBeenCalledWith<[string, string, CookieOptions]>(COOKIE_OPTIONS.ACCESS_TOKEN, accessToken, { httpOnly: true, maxAge: COOKIE_OPTIONS.ONE_HOUR_BY_MS});
            expect(mockRes.cookie).toHaveBeenCalledWith<[string, string, CookieOptions]>(COOKIE_OPTIONS.REFRESH_TOKEN, refreshToken, { httpOnly: true, maxAge: COOKIE_OPTIONS.ONE_WEEK_BY_MS});
        });
    });
    ...
    describe('getAccessFromRefresh, 입력: 응답 객체 & 유저 아이디, 동작: 액세스 토큰 획득 후 쿠키에 저장', () => {
        it('반환: 응답 객체, 조건: 항상', async () => {
            ...
            await expect(controller.getAccessFromRefresh(mockRes as Response, userId)).resolves.toEqual<Response>(mockRes.end());
