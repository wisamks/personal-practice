...
describe('AuthService', () => {
    ...
    describe('oauthLogin, 입력: Oauth 유저 데이터, 동작: 유저 찾아보고 없으면 만들어서 토큰 반환', () => {
        it('반환: 토큰들 객체, 조건: 유저를 못 찾았을 때, 동작: 유저 생성 후 토큰 반환', async () => {
            ...
            expect(userService.getUserOauth).toHaveBeenCalledWith<[IProviderOptions]>({ provider: user.provider, providerId: user.providerId });
            expect(userService.createUser).toHaveBeenCalledWith<[CreateUserReqDto]>({ ...user, password: user.provider });
            ...
            expect(redisClient.set).toHaveBeenCalledWith<[string, string, string, number]>(mockRefreshKey, refreshToken, 'EX', ONE_WEEK_BY_SECOND)
        });
