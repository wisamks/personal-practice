...
export class AuthController {
  ...
  async kakaoCallback(@ReqUser() user: IOauthUserOutput, @Res() res: Response) {
  ...
  @UseGuards(AuthGuard(AUTH_GUARD_STRATEGY.NAVER))
  @Redirect(PATH_CLIENT.DEV)
  async naverCallback(@ReqUser() user: IOauthUserOutput, @Res() res: Response) {
    const { accessToken, refreshToken } = await this.authService.oauthLogin(user);
    res.cookie(COOKIE_OPTIONS.ACCESS_TOKEN, accessToken, {
      httpOnly: true,
      maxAge: COOKIE_OPTIONS.ONE_HOUR_BY_MS,
  ...
  @UseGuards(AuthGuard(AUTH_GUARD_STRATEGY.GOOGLE))
  @Redirect(PATH_CLIENT.DEV)
  async googleCallback(@ReqUser() user: IOauthUserOutput, @Res() res: Response) {
    const { accessToken, refreshToken } = await this.authService.oauthLogin(user);
    res.cookie(COOKIE_OPTIONS.ACCESS_TOKEN, accessToken, {
      httpOnly: true,
      maxAge: COOKIE_OPTIONS.ONE_HOUR_BY_MS,
  ...
  @HttpCode(HttpStatus.NO_CONTENT)
  async signIn(@Body() signInReqDto: SignInReqDto, @Res() res: Response): Promise<Response> {
  ...
  async signOut(@Res() res: Response, @ReqUser('userId') userId: number): Promise<Response> {
  ...
  async getAccessFromRefresh(@Res() res: Response, @ReqUser('userId') userId: number): Promise<Response> {
    ...
    this.logger.verbose(`액세스 토큰 재발급. 유저 아이디: ${userId}`);
    return res.end();
  }
}
