...
describe('PostTagRepository', () => {
  ...
  describe('createRelations, 입력: 트랜잭션 객체 & 여러 게시글 - 태그 관계 객체 배열, 동작: 여러 관계 생성 후 결과 반환', () => {
    it('반환: 생성 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(
        repository.createRelations(mockTx as unknown as Prisma.TransactionClient, data),
      ).resolves.toEqual<Prisma.BatchPayload>(createdResult);
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.createRelations(mockTx as unknown as Prisma.TransactionClient, data)).rejects.toThrow(
  ...
  describe('deleteRelationsByPostId, 입력: 트랜잭션 객체 & 게시글 아이디, 동작: 단일 게시글의 모든 태그 관계 삭제 후 결과 반환', () => {
    it('반환: 삭제 결과 개수 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(
        repository.deleteRelationsByPostId(mockTx as unknown as Prisma.TransactionClient, postId),
