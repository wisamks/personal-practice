...
describe('TagService', () => {
  ...
  describe('createTags, 입력: 트랜잭션 객체 & 태그명 배열 & 게시글 아이디, 동작: 게시글-태그 관계 생성 후 레디스에 저장', () => {
    it('반환: undefined, 조건: 항상, 동작: db에 없는 태그는 생성 후 게시글-태그 관계 생성 및 레디스에 저장', async () => {
      ...
      expect(postTagRepository.createRelations).toHaveBeenCalledWith<[Prisma.TransactionClient, PostTag[]]>(
        tx,
        relations,
      );
      expect(redisClient.set).toHaveBeenCalledWith<[string, string, string, number]>(
  ...
  describe('updateTags, 입력: 트랜잭션 객체 & 태그명 배열 & 게시글 아이디, 동작: 태그 삭제 작업 후 생성 작업', () => {
    it('반환: undefined, 조건: 태그가 있을 때, 동작: 태그 삭제 후 생성 로직 진행', async () => {
      ...
      expect(service.createTags).toHaveBeenCalledWith<[Prisma.TransactionClient, { tags: string[]; postId: number }]>(
        tx,
        { tags, postId },
      );
    });
  ...
  describe('deleteTags, 입력: 트랜잭션 객체 & 게시글 아이디, 동작: 태그 관계 전부 삭제 및 레디스 키 삭제', () => {
    it('반환: undefined, 조건: 항상, 동작: 단일 게시글의 모든 태그 삭제 및 레디스 키 삭제', async () => {
      ...
      expect(postTagRepository.deleteRelationsByPostId).toHaveBeenCalledWith<[Prisma.TransactionClient, number]>(
        tx,
        postId,
      );
    });
