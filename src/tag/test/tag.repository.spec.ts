...
describe('TagRepository', () => {
  ...
  describe('findTagByName, 입력: 트랜잭션 객체 & 태그명, 동작: 단일 태그를 찾아서 반환', () => {
    it('반환: 단일 태그 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(repository.findTagByName(mockTx as unknown as Prisma.TransactionClient, name)).resolves.toEqual<Tag>(
        foundTag,
      );
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.findTagByName(mockTx as unknown as Prisma.TransactionClient, name)).rejects.toThrow(
  ...
  describe('createTagByName, 입력: 트랜잭션 객체 & 태그명, 동작: 단일 태그 객체 생성 후 반환', () => {
    it('반환: 단일 태그 객체, 조건: db 통신 성공', async () => {
      ...
      await expect(
        repository.createTagByName(mockTx as unknown as Prisma.TransactionClient, name),
      ).resolves.toEqual<Tag>(createdTag);
      expect(mockTx.tag.create).toHaveBeenCalledWith<[Prisma.TagCreateArgs]>({
        data,
      });
    });
    ...
    it('반환: BadGateWayError, 조건: db 통신 에러', async () => {
      ...
      await expect(repository.createTagByName(mockTx as unknown as Prisma.TransactionClient, name)).rejects.toThrow(
        RepositoryBadGatewayException,
      );
      expect(mockTx.tag.create).toHaveBeenCalledWith<[Prisma.TagCreateArgs]>({
        data,
